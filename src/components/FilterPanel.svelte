<script lang="ts">
	...
	const yearCounts = $derived(
		new Map(
			years.map((y) => [
				y,
				posts.filter((p) => new Date(p.published).getFullYear() === y).length,
			]),
		),
	);
	const allTags = $derived(aggregateTags(posts).map(([t]) => t));
	const tagCounts = $derived(new Map(aggregateTags(posts)));
